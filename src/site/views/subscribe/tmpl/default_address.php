...
$required = $this->getParams()->get('account.billingAddress');

$fields = array();
if ($required != 'none') {
    $names    = array_keys($this->billing->address->getProperties());
    $required = !$required ? $names : array_intersect($names, explode(',', $required));
    $fields   = $required;
}
if ($fields):
    ?>
