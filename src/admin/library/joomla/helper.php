...
abstract class SimplerenewHelper
{
    ...
    /**
     * Create menu items using xml definition:
     *
     ...
     * @throws Exception
     */
    public  static function createMenu(SimpleXMLElement $menu, $component)
    {
        $componentId = JComponentHelper::getComponent($component)->id;

        $menuType  = array(
            'menutype'    => (string)$menu->menutype,
            'title'       => JText::_($menu->title),
            'description' => JText::_($menu->description)
        );
        $menuItems = $menu->items;

        $table = JTable::getInstance('MenuType');
        if ($table->bind($menuType) && $table->check()) {
            if (!$table->store()) {
        ...
        foreach ($menuItems->children() as $menuItem) {
            ...
            $alias  = SimplerenewApplicationHelper::stringURLSafe((string)$menuItem->alias);
            ...
            while ($table->id) {
                ...
                $alias = SimplerenewApplicationHelper::stringURLSafe($menuItem->alias . '-' . $idx++);

                $table->id = null;
                $table->load(array('alias' => $alias, 'parent_id' => 1));
            }
     ...
     * @throws Exception
     */
    public static function deleteMenu($menuType)
    {
